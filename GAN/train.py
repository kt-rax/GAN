...
from disriminator import Discriminator
...
class Trainer:
    ...
    def load_MNIST(self,model_type = 3):
        ...
        if self.model_type not in allowed_types:
            print('ERROR: only Integer Value from -1 to 9 are allowed')
            
        (self.X_train,self.Y_train),(_,_) = mnist.load_data()
        if self.model_type != -1:
            self.X_train = self.X_train[np.where(self.Y_train == int(self.model_type))]
    ...
    def train(self):
        for e in range(self.EPOCHS):
            ...
            # Step2:Grab Generated Images for this training batch
            ...
            # Step3:Combine to train on the discriminator
            x_batch = np.concatenate([x_real_images,x_generated_images])
            y_batch = np.concatenate([y_real_labels,y_generated_labels])
            
            # Step4:Now,train the discriminitor with this batch
            disriminator_loss = self.discriminator.Discriminator.train_on_batch(x_batch,y_batch)[0]
            
            # Step5:Generate Noise
            x_latent_space_samples = self.sample_latent_space(self.BATCH)
            y_generated_labels = np.ones([self.BATCH,1])            
            generator_loss = self.gan.gan_model.train_on_batch(x_latent_space_samples,y_generated_labels)
            
            # print('Epoch: '+str(int(e))+',[Discriminator()::Loss:'+str(disriminator_loss)+'],[Generator()::Loss:'+str(generator_loss)+']')
